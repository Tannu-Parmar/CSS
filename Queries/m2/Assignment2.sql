...
SELECT SUBSTRING(`PHONE_NO`,7,4) from `Employee`;
...
UPDATE `Employee` set `PHONE_NO`= REPLACE(`PHONE_NO`,123,999);
...
SELECT
    *,
    DAYNAME(`JOINING_DATE`) as 'DAY'
from `Employee`
WHERE
    DAYNAME(`JOINING_DATE`) = 'Monday';
...
FROM `Employee`
WHERE
    DATE(`JOINING_DATE`) BETWEEN '2013-01-01' AND '2013-01-29';
